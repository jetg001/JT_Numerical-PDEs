%% Problem 4 - Poisson Equation with Neumann Boundary Conditions
% This script solves the 2D Poisson equation on [0,L'^2 using a 5-point
% Laplacian stencil with Neumann boundary conditions enforced via ghost
% points. A single interior value is pinned to remove the nullspace.
% Convergence is measured in the l_inf and l_2 norms.
%
% Utilities used:
%   - poisson_neumann_system.m
%   - slanCM.m
...
title('$\ell_\infty$ vs $\ell_2$ Error for Numerical Poisson (Neumann)', 'Interpreter', 'latex');
legend({'$\ell_\infty$ Error', '$\ell_2$ Error', 'Reference Slope $N^{-2}$'}, 'Location', 'northeast', 'Interpreter', 'latex');
saveas(gcf, fullfile('figures', 'p4_neumann_convergence.png'));
hold off;
