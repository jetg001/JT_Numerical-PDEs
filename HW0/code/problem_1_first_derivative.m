...
fp = @(x) cos(x).*exp(sin(x));  % exact derivative

N  = 200;                       % number of gridpoints
x  = linspace(0, 2*pi, N);      % discretized grid
dx = x(2) - x(1);               % discretization size
fx = f(x);                      % exact function solved over discretized grid

fp_num   = first_derivative_fd(fx, dx); % call numerical solution solver
fp_exact = fp(x);               % exact derivative solved over discretized grid

% plot exact vs numerical first derivative
figure;
...
plot(x, fp_num, '--', 'Color', [1 0.5 0], 'LineWidth', 14)      % accessible contrast lines
...
saveas(gcf, fullfile('figures', 'p1_derivative.png')); % automatically save figure (will be full-screen sized)
hold off;
...
for k = 1:length(Ns)
    ...
    err_inf(k) = norm(fp_exact_k - fp_num_k, Inf) / norm(fp_exact_k, Inf); % l-infinity norm
    err_2(k)   = norm(fp_exact_k - fp_num_k, 2)   / norm(fp_exact_k, 2);   % l-2 norm
end
...
loglog(Ns, Ns.^(-1), '--', 'Color', [0.83 0.37 0], 'LineWidth', 10); % reference slopes
loglog(Ns, Ns.^(-3/2), '--', 'Color', [1 0.5 0],'LineWidth', 10);    % all lines accessible contrast
format_loglog_axes();
